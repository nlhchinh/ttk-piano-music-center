...
public class AdSearchController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String cateValue = request.getParameter("cmbCate");
            String status = request.getParameter("cmbStatus");

            CourseDAO cdao = new CourseDAO();
            List<CourseDTO> result = cdao.getCourseBySearchAd(searchValue, cateValue, status);
            if (result.isEmpty()) {
                request.setAttribute("noResult", "No course math your search value!!!");
                request.getRequestDispatcher("adHome.jsp").forward(request, response);
            }
            request.setAttribute("listC", result);
            request.getRequestDispatcher("adHome.jsp").forward(request, response);
        }
    }
